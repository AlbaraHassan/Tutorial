            ...
            return <Grid item xs={3} key={el._id}>
                <Grid container justifyContent="center" spacing={0}>
                    <Card sx={{ minWidth: 275, marginTop: 5, height: 200, backgroundColor: "#f0f8ff" }}>
                        ...
                        <CardActions>
                            {!cart.includes(el) ? <Button size="small" variant="outlined" onClick={() => { handleCart(el) }}>Add To Cart</Button> : <></>}

                            {cart.includes(el) ?

                                <Button size="small" sx={{ color: "red", borderColor: "red" }} variant="outlined" onClick={() => { handleRemove(el) }}>Remove From Cart</Button>
                                : <></>}
                        </CardActions>
                    </Card>
                </Grid>
